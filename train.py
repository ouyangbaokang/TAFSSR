...
def main(args):
    ...
    for idx_epoch in range(start_epoch, args.epoch):
        ...
        ''' Training '''
        loss_epoch_train, psnr_epoch_train, ssim_epoch_train = train(train_loader, device, net, criterion, optimizer,idx_epoch)
...
def train(train_loader, device, net, criterion, optimizer,idx_epoch=None):
    ''' training one epoch '''
    psnr_iter_train = []
    loss_iter_train = []
    ssim_iter_train = []
...
def test(test_loader, device, net, save_dir=None):
    ...
    for idx_iter, (Lr_SAI_y, Hr_SAI_y, Sr_SAI_cbcr, data_info, LF_name) in tqdm(enumerate(test_loader), total=len(test_loader), ncols=70):
        ...
        if save_dir is not None:
            test_name=str(save_dir).split('/')[-1]
            save_dir_ = save_dir.joinpath(LF_name[0])
            if not (os.path.exists('./'+str(save_dir_))):
              os.makedirs('./'+str(save_dir_))
            ...
            for i in range(args.angRes_out):
                for j in range(args.angRes_out):
                    img = Sr_4D_rgb[i, j, :, :, :]
                    # # print(datasz_dict.get(test_name).get(LF_name[0]))
