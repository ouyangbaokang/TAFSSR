...
class get_model(nn.Module):
    def __init__(self, args):
        ...
        self.sr_transformer = SRTransformer(
            1, channels, embed_size=64, heads=n_group*2, num_blocks=n_block)
        self.disentg = CascadeDisentgGroup(
            n_group, n_block, self.angRes, channels)
        self.upsample = nn.Sequential(
            nn.Conv2d(channels, channels * self.factor ** 2,
    ...
    def forward(self, x, info=None):
        x_upscale = F.interpolate(
            x, scale_factor=self.factor, mode='bilinear', align_corners=False)
...
class CascadeDisentgGroup(nn.Module):
    def __init__(self, n_group, n_block, angRes, channels):
        ...
        self.conv = nn.Conv2d(channels, channels, kernel_size=3, stride=1, dilation=int(
            angRes), padding=int(angRes), bias=False)
...
class DisentgBlock(nn.Module):
    def __init__(self, angRes, channels):
        ...
        self.SpaConv = nn.Sequential(
            nn.Conv2d(channels, SpaChannel, kernel_size=3, stride=1,
        ...
        self.AngConv = nn.Sequential(
            nn.Conv2d(channels, AngChannel, kernel_size=angRes,
                      stride=angRes, padding=0, bias=False),
            nn.LeakyReLU(0.1, inplace=True),
            nn.Conv2d(AngChannel, angRes * angRes * AngChannel,
                      kernel_size=1, stride=1, padding=0, bias=False),
            nn.LeakyReLU(0.1, inplace=True),
            nn.PixelShuffle(angRes),
        )
        self.EPIConv = nn.Sequential(
            nn.Conv2d(channels, EpiChannel, kernel_size=[1, angRes * angRes], stride=[
                      1, angRes], padding=[0, angRes * (angRes - 1)//2], bias=False),
            nn.LeakyReLU(0.1, inplace=True),
            nn.Conv2d(EpiChannel, angRes * EpiChannel, kernel_size=1,
    ...
    def forward(self, x):
        ...
        feaEpiV = self.EPIConv(
            x.permute(0, 1, 3, 2).contiguous()).permute(0, 1, 3, 2)
        buffer = torch.cat((feaSpa, feaAng, feaEpiH, feaEpiV), dim=1)
        buffer = self.attention_fusion(buffer)
        return buffer + x
...
class MultiHeadAttention(nn.Module):
    def __init__(self, embed_size, num_heads):
        ...
        assert (self.head_dim * num_heads ==
                embed_size), "Embedding size needs to be divisible by num_heads"
    ...
    def forward(self, values, keys, query, mask=None):
        ...
        energy = torch.einsum(
            "nqhd,nkhd->nhqk", [queries, keys]) / (self.embed_size ** (1 / 2))
...
class SRTransformer(nn.Module):
    def __init__(self, in_channels, out_channels, embed_size, heads, num_blocks):
        ...
        self.conv_in = nn.Conv2d(
            in_channels, embed_size, kernel_size=3, dilation=self.dilation, padding=self.padding)
        self.transformer_blocks = nn.ModuleList(
            [TransformerBlock(embed_size, heads) for _ in range(num_blocks)])
        self.conv_out = nn.Conv2d(
            embed_size, out_channels, kernel_size=3, dilation=self.dilation, padding=self.padding)
...
class AttentionFusion(nn.Module):
    def __init__(self, channels, eps=1e-5):
        ...
        self.fuse = nn.Sequential(
            nn.Conv2d(SpaChannel + AngChannel + 2 * EpiChannel, channels,
                      kernel_size=1, stride=1, padding=0, bias=False),
            nn.LeakyReLU(0.1, inplace=True),
    ...
    def forward(self, x, m_batchsize=4):
        ...
        cov = (1 / (M - 1) * x_reshape @
               x_reshape.transpose(-1, -2)) * self.alpha
        # print(cov)
        norm = cov / ((cov.pow(2).mean((1, 2), keepdim=True) +
                      self.epsilon).pow(0.5))  # l-2 norm

        attention = torch.tanh(self.gamma * norm + self.beta)
        x_reshape = x.view(m_batchsize, N, -1)

        out = torch.bmm(attention, x_reshape)
        out = out.view(N, C, height, width)
        out += x
        out = self.fuse(out)
        return out
